...
def get_chunks(file_uuid: _uuid.UUID):
    return chunks.where(chunks.uuid == file_uuid).select(
        text=chunks.text, title=chunks.title, heading=chunks.heading, page=chunks.page
    )
...
def get_frames(file_uuid: _uuid.UUID, limit: int = 12):
    return (
        video_frames.where(video_frames.uuid == file_uuid)
        .select(frame=video_frames.frame_thumbnail, position=video_frames.pos)
        .limit(limit)
    )
